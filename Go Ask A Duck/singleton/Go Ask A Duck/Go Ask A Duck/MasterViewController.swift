...
//        let mySharedNtwk = SharedNetworking()
//        SharedNetworking.sharedInstance.configSearch(url: urlString) { (dictOfTopics) in
//            
...
//        }
        objects.removeAll()
        SharedNetworking.sharedInstance.getData(url: urlString)
        if SharedNetworking.sharedInstance.error == true {
            showAlert()
            SharedNetworking.sharedInstance.error = false
        } else {
            let cells = SharedNetworking.sharedInstance.getCell()
            for cell in cells {
                objects.append(cell)
//            print("cell")
//            print(cell)
...
//        let mySharedNtwk = SharedNetworking()
//        objects.removeAll()
//        SharedNetworking.sharedInstance.configSearch(url: newUrl) { (relatedTopics) in
////            print(relatedTopics)
//            // Reload the table.  The tables data source should be the property you copied the
//            // issues to (above). Remember to refresh the table on the main thread
//            DispatchQueue.main.async {
...
//        }
//        objects.removeAll()
        SharedNetworking.sharedInstance.getData(url: newUrl)
        if SharedNetworking.sharedInstance.error == true {
            showAlert()
            SharedNetworking.sharedInstance.error = false
        } else {
