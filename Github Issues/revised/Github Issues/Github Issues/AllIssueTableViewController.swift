...
class AllIssueTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //         Configure the cell...
        let cell = self.tableView.dequeueReusableCell(withIdentifier: "allIssues", for: indexPath) as! AllIssueTableViewCell
        
        cell.accessoryType = .disclosureIndicator
        
        cell.title?.text = allIssues[indexPath.row].title
        cell.photo?.image = allIssues[indexPath.row].status  == "open" ? UIImage(named: "open") : UIImage(named: "closed")
        cell.username?.text = allIssues[indexPath.row].author
        cell.date?.text = allIssues[indexPath.row].date
        ...
        let task = session.dataTask(with: url as URL, completionHandler: { (data, response, error) -> Void in
            ...
            do {
                ...
                for json in issues {
                    ...
                    self.allIssues.append(Issue(title: title, author: username, date: date, status: status, comment: comment, body: body, url: url))
                }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        idvc?.passedIssueData = allIssues[index!]
    }
